...
const checkExistingProducts = async () => {
  ...
  try {
    const productNames = products.value.filter((p) => p.name).map((p) => p.name)
...
const validateBatch = () => {
  ...
  products.value.forEach((product, index) => {
    ...
    if (product.brand === '__custom' && !product.customBrand.trim())
      errors.push(`Product ${index + 1}: Custom brand name is required`)
...
<template>
  <div class="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-10 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div class="text-center lg:text-left">
          ...
          <p class="text-slate-500 font-medium italic">
            Upload multiple products to Bitrix24 & Cloudinary simultaneously
          </p>
        </div>
        ...
        <div class="flex flex-wrap justify-center lg:justify-end items-center gap-3 lg:max-w-[50%]">
          ...
          >
            <span class="material-symbols-outlined text-sm">{{
              currentAdmin?.is_master ? 'person_add' : 'lock_reset'
            }}</span>
      ...
      <form class="space-y-8" @submit.prevent="submitBatch">
        <!-- Products Container -->
        <div class="space-y-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-slate-500 text-sm mt-1 font-medium">
                {{ products.length }} product{{ products.length !== 1 ? 's' : '' }} in queue
              </p>
            </div>
          ...
          <!-- Product Cards Grid -->
          <transition-group name="list" tag="div" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div
              v-for="(product, productIndex) in products"
              :key="product.id"
              class="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-8 space-y-8 relative transition-all duration-500"
              :class="{
                'border-orange-300 bg-orange-50/20 scale-[1.01] shadow-xl shadow-orange-500/5':
                  getDuplicatesInBatch()[product.id] || existingProducts[product.id],
            ...
            >
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 flex-wrap">
                    <span
                      class="px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-[10px] font-black uppercase tracking-widest"
                      >Item #{{ productIndex + 1 }}</span
                    >
                    <h3 class="text-xl font-black text-slate-900">{{ product.name || 'Untitled Product' }}</h3>

                    <div class="flex gap-2">
                      <div
                        v-if="getDuplicatesInBatch()[product.id]"
                        class="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 rounded-lg text-[10px] font-black uppercase"
                      >
                        <span class="material-symbols-outlined text-xs">warning</span>
                        Batch Match
                      </div>
                      <div
                        v-if="existingProducts[product.id]"
                        class="inline-flex items-center gap-1 px-2 py-1 bg-amber-100 text-amber-700 rounded-lg text-[10px] font-black uppercase"
                      >
              ...
              <div
                v-if="uploadProgress[product.id]"
                class="absolute top-8 right-32 flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-sm"
                :class="{
              ...
              >
                <span
                  v-if="uploadProgress[product.id] === 'uploading'"
                  class="animate-spin inline-block w-3 h-3 border-2 border-white border-t-transparent rounded-full"
                />
                {{
                  uploadProgress[product.id] === 'uploading'
                    ? 'Publishing'
                    : uploadProgress[product.id] === 'success'
                      ? 'Live'
                      : 'Failed'
                }}
              </div>
              ...
              <div class="grid grid-cols-2 gap-6">
                <div class="col-span-2 sm:col-span-1">
                  <label
                    class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 leading-none"
                    >Product Title</label
                  >
                  <input
              ...
              <!-- Description -->
              <div>
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 leading-none"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  <label
                    class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 leading-none"
                    >Cover Image</label
                  >
                  ...
                  >
                    <template v-if="product.image">
                      ...
                      >
                        <button
                          type="button"
                          class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-red-500 transition-all flex items-center justify-center"
                ...
                <div>
                  <div class="flex items-center justify-between mb-3">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none"
                      >Visual Gallery</label
                    >
                    <button
                      v-if="product.gallery.length"
                      type="button"
                      class="text-[9px] font-black uppercase text-red-500 hover:underline"
                      @click="clearGallery(product.id)"
                    >
                      Clear All
                    </button>
                  </div>
                  ...
                  >
                    <div v-if="product.gallery.length" class="p-3 grid grid-cols-2 gap-2">
                      <div
                        v-for="(file, idx) in product.gallery"
                        :key="idx"
                      ...
                      <!-- Quick Add Slot -->
                      <div
                        class="relative aspect-square rounded-xl border-2 border-dashed border-slate-200 flex items-center justify-center bg-white/50 hover:bg-white transition-all cursor-pointer"
                      >
                        <input
                          type="file"
                          multiple
        ...
        <!-- Final Publish Bar -->
        <div
          class="fixed bottom-10 left-1/2 -translate-x-1/2 z-40 bg-white/80 backdrop-blur-xl px-10 py-6 rounded-[2.5rem] shadow-2xl shadow-slate-900/10 border border-white/50 flex items-center gap-10"
        >
          <div class="hidden sm:block">
            ...
            <p class="font-black text-slate-900 text-lg">
              Ready to Launch <span class="text-blue-600">({{ products.length }} Items)</span>
            </p>
          </div>
          ...
          >
            ...
            />
            <span v-else class="material-symbols-outlined text-2xl group-hover:rotate-12 transition-transform"
              >rocket_launch</span
            >
