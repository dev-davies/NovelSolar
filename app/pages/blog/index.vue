  ...
  <div class="min-h-screen bg-white">
    ...
    <section class="bg-slate-50 py-16">
      <div class="max-w-7xl mx-auto px-4 text-center sm:text-left">
        <h1 class="text-4xl font-black text-[#002888] mb-4 tracking-tight">Novel Solar Insights</h1>
        <p class="text-lg text-slate-600 max-w-2xl leading-relaxed font-medium">
          Stay informed with the latest in solar technology, industry innovations, and expert advice for a sustainable
          energy future.
        </p>
      </div>
    </section>
    ...
    <main class="max-w-7xl mx-auto px-4 py-16">
      <!-- Loading State -->
      <div v-if="pending" class="py-20 text-center text-slate-500 font-medium">Loading insights...</div>
          ...
          >
            ...
            <div class="p-8 flex flex-col flex-grow">
              <div class="flex items-center justify-between mb-4">
                <span class="text-xs font-bold text-[#002888] uppercase tracking-widest">{{
                  formatDate(post.date)
                }}</span>
                <span
                  v-if="post.category"
                  class="bg-blue-50 text-[#002888] text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider"
                  >{{ post.category }}</span
                >
              </div>

              <h3
                class="text-xl font-bold text-slate-900 mb-4 group-hover:text-[#002888] transition-colors line-clamp-2 leading-tight"
              >
                {{ post.title }}
              </h3>
      ...
      <div v-else class="text-center py-20 border-2 border-dashed border-slate-200 rounded-2xl">
        ...
        <h3 class="text-2xl font-black text-slate-900 mb-3 uppercase tracking-tighter italic">
          First Insights Coming Soon
        </h3>
        <p class="text-slate-500 max-w-sm mx-auto font-medium">
          We're currently preparing our first batch of high-performance solar insights. Check back shortly!
        </p>
  ...
  meta: [
    {
      name: 'description',
      content:
        'Explore latest solar technology insights, maintenance guides, and sustainable energy news from the experts at Novel Solar.',
    },
  ],
})
</script>
