    ...
    <div class="flex flex-col lg:flex-row gap-8">
      ...
      <aside class="lg:w-1/4 space-y-8">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
          ...
          <button
            class="bg-white border border-gray-200 text-gray-600 w-full mt-6 py-2 px-4 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors"
            @click="resetFilters"
          >
            Clear All
          </button>
      ...
      <main class="lg:w-3/4">
        <!-- Grid Header -->
        <div
          class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100"
        >
          <div>
          ...
          <div class="flex items-center gap-4 w-full sm:w-auto">
            ...
            <select
              class="block w-full sm:w-48 pl-3 pr-10 py-2 text-sm border-gray-200 focus:outline-none focus:ring-primary focus:border-primary rounded-md bg-gray-50/50"
            >
        ...
        <div v-else class="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
          ...
          <div class="mt-6">
            <button
              class="bg-[#002888] text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-blue-900 transition-colors shadow-sm"
              @click="resetFilters"
            >
              Reset all filters
            </button>
...
const filteredProducts = computed(() => {
  return products.filter((p) => {
    return (
      p.price <= filters.value.priceRange &&
      p.wattage <= filters.value.wattage &&
      p.efficiency >= filters.value.efficiency - 5
