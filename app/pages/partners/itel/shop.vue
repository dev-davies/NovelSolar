  ...
  <div class="min-h-screen bg-gray-50 p-8 lg:p-12">
    <div class="max-w-7xl mx-auto">
      ...
      <div v-else-if="itelProducts && itelProducts.length > 0" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div v-for="product in itelProducts" :key="product.ID" class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col relative overflow-hidden">
          <!-- Official Badge -->
          ...
          <div class="aspect-square bg-gray-50 flex items-center justify-center border-b border-gray-50 overflow-hidden relative">
            ...
            <div class="absolute inset-0 bg-[#002888]/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <NuxtLink :to="'/products/' + product.ID" class="bg-white text-[#002888] px-4 py-2 rounded-lg text-xs font-bold shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform">
                View Specs
              </NuxtLink>
            </div>
          </div>
