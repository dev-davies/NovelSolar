...
const TestAdminManagement = defineComponent({
  template: `
...
describe('AdminManagement', () => {
  ...
  it('updates form data on input', () => {
    ...
    expect((wrapper.vm as any).newAdminForm.email).toBe('newadmin@example.com')
    expect((wrapper.vm as any).newAdminForm.username).toBe('new_admin')
  })

  it('disables create button when email is empty', () => {
    const wrapper = mount(TestAdminManagement)
    ;(wrapper.vm as any).newAdminForm = { email: '', username: 'test' }
  ...
  it('disables create button when username is empty', () => {
    const wrapper = mount(TestAdminManagement)
    ;(wrapper.vm as any).newAdminForm = { email: 'test@example.com', username: '' }
  ...
  it('loads admins on refresh click', async () => {
    mockFetch.mockResolvedValueOnce({
      admins: [{ id: '3', username: 'new_admin', email: 'new@example.com', is_master_admin: false }],
    })
  ...
  it('shows loading state while loading admins', async () => {
    ...
    expect((wrapper.vm as any).isLoadingAdmins).toBe(true)
  })
  ...
  it('shows error message on load failure', async () => {
    ...
    expect((wrapper.vm as any).loadError).toBe('Failed to load')
  })
  ...
  it('shows success message after creating admin', async () => {
    ...
    expect((wrapper.vm as any).createAdminSuccess).toBe('Admin created successfully!')
  })
  ...
  it('shows error message on create failure', async () => {
    ...
    expect((wrapper.vm as any).createAdminError).toBe('Failed to create')
  })
  ...
  it('resets form after successful creation', async () => {
    ...
    expect((wrapper.vm as any).newAdminForm.email).toBe('')
    expect((wrapper.vm as any).newAdminForm.username).toBe('')
  })
})
