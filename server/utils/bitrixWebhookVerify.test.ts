...
describe('verifyBitrixApplicationToken', () => {
  ...
  it('allows requests and warns when token is not configured', () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('Bitrix Webhook', expect.stringContaining('not configured'))
  })
  ...
  it('rejects requests with wrong application_token (flat)', () => {
    const result = verifyBitrixApplicationToken({ AUTH_ID: 'abc', application_token: 'wrong-token' }, 'my-secret-token')
  ...
  it('uses timing-safe comparison (different length tokens rejected)', () => {
    const result = verifyBitrixApplicationToken({ application_token: 'short' }, 'much-longer-expected-token')
